...
function registerLLMHandlers() {
  electron.ipcMain.on(IPC.LLM_STREAM_START, async (event, messages, config) => {
  ...
  electron.ipcMain.on(IPC.LLM_ABORT, () => {
    llmService.abort();
  });
}
const projectsDir = path.join(electron.app.getPath("userData"), "projects");
async function ensureProjectsDir() {
  if (!fs.existsSync(projectsDir)) {
    await promises.mkdir(projectsDir, { recursive: true });
  }
}
async function saveFileDialog(content2, defaultPath) {
  const result = await electron.dialog.showSaveDialog({
    defaultPath: defaultPath || "architecture.md",
    filters: [{ name: "Markdown", extensions: ["md"] }]
  });
  if (result.canceled || !result.filePath) return null;
  await promises.writeFile(result.filePath, content2, "utf-8");
...
async function openFileDialog() {
  ...
  return { content: content2, path: filePath };
}
async function exportMDDialog(content2) {
  const result = await electron.dialog.showSaveDialog({
...
async function saveProject(id2, data) {
  await ensureProjectsDir();
  const filePath = path.join(projectsDir, `${id2}.json`);
  await promises.writeFile(filePath, JSON.stringify(data, null, 2), "utf-8");
}
async function loadProject(id2) {
  const filePath = path.join(projectsDir, `${id2}.json`);
  const content2 = await promises.readFile(filePath, "utf-8");
...
async function listProjects() {
  await ensureProjectsDir();
  const files = await promises.readdir(projectsDir);
  const projects = [];
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    try {
      const content2 = await promises.readFile(path.join(projectsDir, file), "utf-8");
...
async function deleteProject(id2) {
  const filePath = path.join(projectsDir, `${id2}.json`);
...
function registerFileHandlers() {
  electron.ipcMain.handle(IPC.FILE_SAVE, async (_event, content2, defaultPath) => {
    return saveFileDialog(content2, defaultPath);
  });
  electron.ipcMain.handle(IPC.FILE_OPEN, async () => {
    return openFileDialog();
  });
  electron.ipcMain.handle(IPC.FILE_EXPORT, async (_event, content2) => {
    return exportMDDialog(content2);
  });
  electron.ipcMain.handle(IPC.PROJECT_SAVE, async (_event, id2, data) => {
    return saveProject(id2, data);
  });
  electron.ipcMain.handle(IPC.PROJECT_LOAD, async (_event, id2) => {
    return loadProject(id2);
  });
  electron.ipcMain.handle(IPC.PROJECT_LIST, async () => {
    return listProjects();
  });
  electron.ipcMain.handle(IPC.PROJECT_DELETE, async (_event, id2) => {
    return deleteProject(id2);
  });
}
function registerSettingsHandlers() {
  electron.ipcMain.handle(IPC.SETTINGS_GET, (_event, key) => {
    return getSetting(key);
  });
  electron.ipcMain.handle(IPC.SETTINGS_SET, (_event, key, value) => {
